...
import { FaEdit, FaTrash, FaPlus, FaSignOutAlt, FaUserGraduate, FaUser, FaBook, FaUserTie, FaChartBar, FaCog } from 'react-icons/fa';
    ...
    <div className="flex flex-col lg:flex-row">
      ...
      <div className="flex-1 p-8 bg-gray-100 min-h-screen lg:ml-64">
              ...
              {courses.map((course) => (
                <tr key={course._id} className="text-center">
                  ...
                  <td className="border border-gray-300 p-2">
                    <button onClick={() => handleEdit(course)} className="bg-yellow-500 text-white px-2 py-1 rounded mr-2"><FaEdit /></button>
                    <button onClick={() => handleDelete(course._id)} className="bg-red-500 text-white px-2 py-1 rounded"><FaTrash /></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <form onSubmit={handleSubmit} className="mt-6">
          ...
          <button type="submit" className="bg-green-500 text-white px-4 py-2 rounded">{editingCourse ? 'Update Course' : 'Add Course'}</button>
          {editingCourse && (
            <button onClick={() => setEditingCourse(null)} className="bg-gray-500 text-white px-4 py-2 rounded ml-2">Cancel</button>
          )}
        </form>
      </div>
    </div>
